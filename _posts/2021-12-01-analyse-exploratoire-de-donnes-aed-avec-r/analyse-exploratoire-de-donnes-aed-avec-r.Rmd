...
description: |
  Les données brutes doivent être inspectées , explorées pour de nouvelles hypothèses et réparées afin d'augmenter la qualité et la sortie des données.
author:
...
library(kableExtra) # création de table de données
library(extrafont) #
library(hrbrthemes)
extrafont::loadfonts()
```
